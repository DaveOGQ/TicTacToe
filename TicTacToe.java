...
public class TicTacToe extends Canvas {
    ...
    /**
     * integer amount for the number of consecutive pieces to reach a win
     */
    public static final int winReached = 3;
    ...
    //createBoard
    /**
     * Creating a board for the pieces to be played on
     * @param rows int value of rows wanted in the board
     * @param columns int value of columns wanted in the board
     * @return 2D array with rows as the first dimension and columns as the second
     */
    ...
    //rowsIn
    /**
     * Checks how many rows are in a given board
     * @param board 2D array being checked
     * @return int value of rows in the board
     */
    ...
    //columnsIn
    /**
     * Checks how many columns are in a given board
     * @param board 2D array being checked
     * @return int value of columns in the board
     */
     ...
     * @param board 2D array to be checked
     * @param row int index value of the index of the row of position
     * @param column int value of the index of the column of the position
     * @return returns true if position is empty and false otherwise
     */
    ...
    //play
    /**
     * places a piece on a specified position on board
     * @param board 2D array which the pieces will be stored in
     * @param row int index value of the index of the row of position
     * @param column int value of the index of the column of the position
     * @param piece piece to be stored in the array, X or O
     */
     ...
     * @param board 2D array to be checked
     * @return returns false if any position is empty and true otherwise
     */
    ...
    //winInRow
    /**
     * Checks for a win with minimum 3 consecutive identical pieces in a given row
     * @param board 2D array to be checked
     * @param row int value of the index of the row to be checked
     * @param piece int piece for which the win is being checked for, X or O
     * @return True if there is a win in the row and false otherwise
     */
    public static boolean winInRow(int[][] board, int row, int piece){
        int winCount = 0;
        for(int column: board[row]){
            if(column == piece){
                winCount ++;
                if(winCount == winReached){
                    return true;
                }
            }
            else{
    ...
    //winInColumn
    /**
     * Checks for a win with minimum 3 consecutive identical pieces in a given column
     * @param board 2D array to be checked
     * @param column int value of the index of the column to be checked
     * @param piece int piece for which the win is being checked for, X or O
     * @return True if there is a win in the column and false otherwise
     */
    public static boolean winInColumn(int[][] board, int column, int piece){
        int winCount = 0;
        for(int row = 0; row < board.length; row++){
            //loop below checks if each position is equal to the piece and adds 1 to the winCount and
            // returns true once it equals 3, however if a different piece or an empty position appears the winCount is reset to zero
    ...
    //winInDiagonalBS
    /**
     * Checks for a win with minimum 3 consecutive identical pieces in any backslash diagonal
     * Every single backslash diagonal is explored in the code below,
     * the number of starting points for each diagonal is the number of rows plus the number of columns
     * excluding one(the corner that rows and columns share). I've created two loops, one that explores the diagonal starting along the rows
     * and another for the diagonals starting along the columns.
     * @param board 2D array to be checked
     * @param piece int piece for which the win is being checked for, X or O
     * @return True if there is a win in the backslash diagonal and false otherwise
     */
    public static boolean winInDiagonalBS(int[][] board, int piece){
        int rows = rowsIn(board);
        int columns = columnsIn(board);

        // Scaling right Side from first row to the last row
        ...
        while(rowStart < rows){
            ...
            int currentColumn = indexColumn;
            while((currentRow >= 0) & (currentColumn >= 0)){ // if my diagonals start on the right side and bottom row, then they must end on the right side and top row
                //loop below checks if each position is equal to the piece and adds 1 to the winCount and
                // returns true once it equals 3, however if a different piece or an empty position appears the winCount is reset to zero
        ...
        //Scaling bottom from first column till second last column
        while(columnStart < indexColumn){
            int winCount = 0;
            //start position one
            int currentColumn = columnStart;
            int currentRow = indexRow;
            while((currentColumn >= 0) & (currentRow >= 0)){// if my diagonals start on the right side and bottom row, then they must end on the left side and top row
                //loop below checks if each position is equal to the piece and adds 1 to the winCount and
                // returns true once it equals 3, however if a different piece or an empty position appears the winCount is reset to zero
                if(board[currentRow][currentColumn] == piece){
                    winCount ++;
    ...
    /**
     * Checks for a win with minimum 3 consecutive identical pieces in any forward slash diagonal
     * Every single forward slash diagonal is explored in the code below,
     * the number of starting points for each diagonal is the number of rows plus the number of columns
     * excluding one(the corner that rows and columns share). I've created two loops, one that explores the diagonal starting along the rows
     * and another for the diagonals starting along the columns.
     * @param board 2D array to be checked
     * @param piece int piece for which the win is being checked for, X or O
     * @return True if there is a win in the forward slash diagonal and false otherwise
     */
    public static boolean winInDiagonalFS(int[][] board, int piece){
        ...
        int indexColumn = columns - 1;
        int rowStart = 0;
        //scaling left side from first row till last row
        while(rowStart < rows){
            int winCount = 0;
            //start position one
            int currentRow = rowStart;
            int currentColumn = 0;
            //Scaling left side from first row till last row
            while((currentRow >= 0) & (currentColumn <= indexColumn)){// if my diagonals start on the left side and bottom row, then they must end on the right side and top row
                //loop below checks if each position is equal to the piece and adds 1 to the winCount and
                // returns true once it equals 3, however if a different piece or an empty position appears the winCount is reset to zero
                if(board[currentRow][currentColumn] == piece){
                    winCount ++;
        ...
        //Scaling bottom for second column till last column
        int indexRow = rows - 1;
        int columnStart = 1;
    ...
    //hint
    /**
     * Checks the entire board looking for possible positions that result in a win
     * @param board 2D array being checked
     * @param piece int piece for which the win is being checked for, X or O
     * @return 1D array of the indexes of the position that results in a win, and a default of {-1,-1}, if no position result in a win
     */
